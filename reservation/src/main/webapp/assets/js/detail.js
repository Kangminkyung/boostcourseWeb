...
document.addEventListener("DOMContentLoaded", () => {
	promotionObj.initPromotion(); // 상단 이벤트 슬라이드 
	itemObj.initItem(); // 상품 정보
	reservationObj.initReservation(); // 예매하기  + 한줄평
	tabObj.initTab(); // tab
});
...
const promotionObj = {
	...
	showProductDetail : function(xhr){
		let res = xhr.response;
		promotionObj.showTitleImages(res);// 1. 타이틀이미지
	},

	showTitleImages : function(res){
	...
	slideEvent : function(productImages){
		document.querySelector(".btn_nxt").addEventListener("click", this.slideImage);
		document.querySelector(".btn_prev").addEventListener("click", this.slideImage);
	},
	
	slideImage : function(click){
		if (flag) {
			flag = false;
...
const itemObj = {
	...
	showProductDetail : function(xhr){
		...
		document.querySelector(".bk_more._open").addEventListener("click", itemObj.readMore); // 2-1. 펼쳐보기
		document.querySelector(".bk_more._close").addEventListener("click", itemObj.readHide); // 2-2. 접기
		
		itemObj.showProductContent(res.displayInfo.productContent);  // 2-3. 펼쳐보기 상세설명
		itemObj.showEvent(res.displayInfo.productEvent);// 3. 이벤트
	...
	// 2-3. 펼쳐보기 상세정보
	showProductContent: function(productContent){
		document.querySelector(".store_details .dsc").innerHTML += productContent;
	},
	
	// 3. 이벤트정보(jsp 구현)
	showEvent: function(productEvent){
...
const reservationObj = {
	...
	showProductDetail : function(xhr){
		...
		document.querySelector(".section_btn").addEventListener("click", reservationObj.reservate); // 4. 예약하기
		reservationObj.getAverageScore(res.averageScore); // 5-1. 평점
		reservationObj.getComments(res.comments); // 5-2. 한줄평
	},
...
const tabObj = {
		...
		showProductDetail : function(xhr){
			...
			document.querySelector(".info_tab_lst").addEventListener("click", tabObj.changeTab); // 상세페이지 탭 변경
			tabObj.getIntroduce(res); // 6-1. 상세정보
			tabObj.getLocation(res.displayInfoImages, res.displayInfo); // 6-2. 오시는길
		},
		...
		// 6-2. 오시는길
		getLocation : function(displayInfoImages, displayInfo){
